...
from .prompts import (
    ...
    get_interpretation_system_prompt,
    get_interpretation_user_prompt,
    get_error_system_prompt,
    get_error_user_prompt,
)
from .utils import clean_sql_query, is_valid_sql_structure, is_safe_sql
...
class ChatService:
    ...
    def _initialize_db_executor(self):
        ...
        logger.info(f"Using database connection string pattern: postgresql://user:password@{db_host}:{db_port}/{db_name}")
    ...
    def _initialize_model_registry(self):
        ...
        logger.info(f"Allowed tables from model registry: {', '.join(self.allowed_tables)}")
    ...
    def _build_messages_for_llm(self, conversation_id: str, user_query: str = "") -> list:
    ...
    def _generate_and_execute_sql(self, user_query, schema, conversation_id):
        ...
        try:
            ...
            # Generate SQL query
            system_prompt = get_sql_generation_system_prompt(schema, self.allowed_tables)
            user_prompt = get_sql_generation_user_prompt(user_query)
            
            messages = conversation_messages + [{"role": "user", "content": user_prompt}]
            ...
            if combined_sql.startswith("I cannot answer this query with the available data and tools."):
    ...
    def _interpret_sql_results(self, user_query, sql_query, raw_result, schema, conversation_id):
        ...
        try:
            ...
            system_prompt = get_interpretation_system_prompt(schema, user_query)
            user_prompt = get_interpretation_user_prompt(user_query, sql_query, raw_result)
            
            messages = interpretation_messages + [{"role": "user", "content": user_prompt}]
            ...
            self.save_message(conversation_id, "system", f"SQL: {sql_query}\nResult: {raw_result}")
            ...
            error_message = f"Error interpreting results: {str(e)}\n\nRaw result: {raw_result}"
            self.save_message(conversation_id, "assistant", error_message)
            return {
                "reply": error_message,
                "sql_query": sql_query,
    ...
    def handle_sql_error(self, user_query, sql_query, error_message, conversation_id):
        ...
        try:
            ...
            system_prompt = get_error_system_prompt()
            user_prompt = get_error_user_prompt(sql_query, error_message)
