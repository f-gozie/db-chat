...
def is_safe_sql(text, allowed_tables):
    ...
    if re.search(r'\b(insert|update|delete|drop|alter|create|truncate)\b', text, re.IGNORECASE):
        logger.warning(f"SQL contains write operations: {text}")
        return False
    
    # Extract table names from the query
    from_clauses = re.findall(r'\bfrom\s+(\w+)(?:\s+(?:as\s+)?(\w+))?', text, re.IGNORECASE)
    join_clauses = re.findall(r'\b(?:inner|left|right|outer|cross)?\s*join\s+(\w+)(?:\s+(?:as\s+)?(\w+))?', text, re.IGNORECASE)
